(ns boomi.main
  "Kotodama WASM entrypoint for the Boomi clean-room actor (L4) — Clojure port.
  ...
  facts (`boomi.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"boomi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boomi-compat" :tier tier :entities entities} 200])
